 ...
 */
define(['ORO/mavensearch', 'N/record', 'N/search', './lib'],
    /**
     * @param{mavenSearch} mavenSearch
     * @param{record} record
     * @param{search} search
     * @param{lib} lib
     */
    ...
    * */
    ...
    (mavenSearch, record, search, lib) => {

        // MDIMKOV 14.10.2021: this function uses CloudMaven SuiteTax calculation to get the net amount out of the gross amount
        function getNetAmount(customerId, itemId, grossAmount) {
            let itemIds = [];
            itemIds.push(itemId);
            let taxRates = mavenSearch.getTaxRate(customerId, itemIds);
            return (100 * grossAmount) / (100 +taxRates[0]);
        }
        ...
        const put = (requestBody) => {
            ...
            try {
                ...
                salesInvoiceData.forEach((salesInvoice) => {
                    ...
                    log.debug('salesInvoice', salesInvoice);
                    ...
                    try {
                        ...
                        if (bookCode && currency) {
                            ...
                            const customerId = lib.getRecIdbyExtId('customer', bookCode);
                            recSalesInvoice.setValue('entity', customerId);
                            ...
                            salesInvoice.ARInvoiceLine.forEach((salesInvoiceLine) => {
                                ...
                                const grossAmount = salesInvoiceLine.Amount;
                                ...
                                const amount = getNetAmount(customerId, itemId, grossAmount)
